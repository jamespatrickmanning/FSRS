...
##  HARDCODES ###
year = '2011'  # where this is the start year and the data usually strattles the new year
LFAzone = '34'
pc_or_linux = 'pc'  # directory names differ depending on which machine I am working on
if pc_or_linux == 'pc':
    inputdir = 'C:/Users/carle/BDC/eMOLT/FSRS/Jim_Manning/FSRS_data_2011_2012/LFA 34/'  # this is the directopry with all the csv &  txt files for this year and LFAzone
...
for j in sns:  # loop through all the serieal numbers and look to read the appropriate csv file
    ...
    if (len(sorted(glob.glob(inputdir + 'Minilog-T_' + str(int(j)) + '*.csv'))) > 0) or (len(sorted(
            glob.glob(inputdir + 'Asc-' + str(int(j)) + '*.000'))) > 0):  # if a csv file exist for this serial number
