...
##  HARDCODES ###
year = '2013'  # where this is the start year and the data usually strattles the new year
LFAzone = '34'
pc_or_linux = 'pc'  # directory names differ depending on which machine I am working on
if pc_or_linux == 'pc':
    inputdir = 'C:/Users/carle/BDC/eMOLT/FSRS/Jim_Manning/FSRS_data_2013_2014/'  # this is the directopry with all the csv &  txt files for this year and LFAzone
...
def clean_data_BDC(df, lalo, d, mean_std, IQR, titlelabel):
    ...
    if IQR.std() > 1:
        ...
        if len(df_5[df_5['first_time'] > 1.5]) == 0:
        ...
        if 'Mark Jeffrey' in titlelabel:
            last_date = df_5[df_5['last_time'] > 1.8].index[-1] + timedelta(days=2)
        elif 'Nelson Ross' in titlelabel:
            last_date = df_5[df_5['last_time'] > 1.5].index[-2]
...
inputdir = inputdir + '20132014 CSV/LFA ' + LFAzone + '/'
