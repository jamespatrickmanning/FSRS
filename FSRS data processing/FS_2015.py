...
##  HARDCODES ###
year = '2015'  # where this is the start year and the data usually strattles the new year
LFAzone = '34'
pc_or_linux = 'pc'  # directory names differ depending on which machine I am working on
if pc_or_linux == 'pc':
    inputdir = 'C:/Users/carle/BDC/eMOLT/FSRS/Jim_Manning/FSRS_data_2015_2016/20152016 files/'  # this is the directopry with all the csv &  txt files for this year and LFAzone

latlon_file = inputdir + 'LR Results 15 16/LatLong LFA ' + LFAzone + '_' + year[2:4] + str(
    int(year[2:4]) + 1) + '.xlsx'  # standard log file for this LFA and year
...
print(latlon_file)
lalo_all = pd.read_excel(
    latlon_file)  # .dropna()#,parse_dates={'datet':[1]},index_col='datet',date_parser=parsedate) # read in the tab-delimited header info
...
sns = set(lalo_all['gauge'])  # finds the unique serial numbers in the header file
# keeps only those non-nan
