...
##  HARDCODES ###
year = '2016'  # where this is the start year and the data usually strattles the new year
LFAzone = '34'
pc_or_linux = 'pc'  # directory names differ depending on which machine I am working on
if pc_or_linux == 'pc':
    inputdir = 'C:/Users/carle/BDC/eMOLT/FSRS/Jim_Manning/FSRS_data_2016_2017/LRdata_20162017/'  # this is the directopry with all the csv &  txt files for this year and LFAzone

latlon_file = inputdir + 'LFAtextfiles/LatLong LFA ' + LFAzone + '_' + year[2:4] + str(
...
def parse(datet, hrmn):  # parses dates in the asc file
    try:
        ...
        dt = datetime.strptime(datet[0:10], '%d/%m/%Y')  # normal format
    try:
        delta = timedelta(hours=int(hrmn[0:2]), minutes=int(hrmn[3:5]), seconds=int(hrmn[6:8]))
    except:
        delta = timedelta(hours=int(hrmn[0:1]), minutes=int(hrmn[3:5]), seconds=int(hrmn[6:8]))
    return dt + delta
...
inputdir = inputdir + 'LFA Temps csv. Only/LFA34Temp/'
