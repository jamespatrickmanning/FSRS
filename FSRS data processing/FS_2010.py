...
LFAzone = '34' # Set zone we want to process
pc_or_linux = 'pc'  # directory names differ depending on which machine I am working on
if pc_or_linux == 'pc':
    inputdir = 'C:/Users/carle/BDC/eMOLT/FSRS/Jim_Manning/FSRS_data_2010_2011/LFA34/Minilogdata/'  # this is the directopry with all the csv &  txt files for this year and LFAzone

latlon_file = inputdir + 'LatLong LFA ' + LFAzone + '_' + year[2:4] + str(
...
def clean_data_BDC(df, lalo, d, mean_std, IQR, titlelabel):
    ...
    if IQR.std() > 1:
        ...
        first_date = df_5[df_5['first_time'] > 3].index[0]
        last_date = df_5[df_5['last_time'] > 3].index[-1]

        display(df_5[df_5['first_time'] > 3])
        if IQR.max() < 15:
...
for j in sns:  # loop through all the serieal numbers and look to read the appropriate csv file
    ...
    if (len(sorted(glob.glob(inputdir + 'Minilog-T_' + str(int(j)) + '*.csv'))) > 0) or (
            len(sorted(glob.glob(inputdir + 'Asc-' + str(int(j)) + '*.000'))) > 0) or (len(sorted(
            glob.glob(inputdir + 'Asc-' + str(int(j)) + '*.001'))) > 0):  # if a csv file exist for this serial number
        print('Minilog file is', 'Minilog-T_' + str(int(j)) + '*.csv')
        try:
            asc_file = sorted(glob.glob(inputdir + 'Asc-' + str(int(j)) + '*.001'))[0]
        except:
            asc_file = sorted(glob.glob(inputdir + 'Asc-' + str(int(j)) + '*.000'))[
                0]  # if it is not a "Minilog-T" file, it might be available as a "Asc-" file.

        f = open(asc_file,
                 'rb')  # reads the Minilog ascii file where we need this 'rb' in Python3 or we'll get a "utf-8" error
        lines = f.readlines()

        start_time = lines[3].decode('utf-8').split('=')[-1].split(',')[0]
        start_time = datetime.strptime(start_time, '%Y-%m-%d')
        local_time = 4 if nov_change <= start_time and start_time < march_change else 3
        ...
        if 'Description' in lines[2].decode('utf-8'):  # as in 2011 cases
            fisherman = lines[2][19:-2].decode('utf-8').replace(' ',
                                                                '_')  # fishermen's name is usually here in 2012 files field
        else:
            fisherman = lines[2][11:-2].decode('utf-8').replace(' ',
                                                                '_')  # fishermen's name is usually here in 2011 files
        f.close()
        ...
        df = pd.read_csv(asc_file, skiprows=skipr, parse_dates={'datet': [0, 1]}, index_col='datet', date_parser=parse,
                         names=['Date', 'Time', 'temp_logged'],
                         encoding='unicode_escape')  # dataframe with datetime index and tempC
